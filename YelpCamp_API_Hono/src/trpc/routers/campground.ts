...
export const campgroundRouter = router({
  ...
  /**
   * Create campground (authenticated users only)
   */
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.data.price) {
        updateData.price = input.data.price.trim()
      }
      if (input.data.image) {
        updateData.image = input.data.image.trim()
      }
      if (input.data.description) {
        updateData.description = input.data.description.trim()
      }
      if (input.data.location !== undefined) {
        updateData.location = input.data.location?.trim() || null
      }
